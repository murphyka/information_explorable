...
window.initInfoTelegraph = async function({sel, state, isBig=true}){
  ...
  async function drawTelegraphPlane (){
    ...
    for (let i=0; i<(state.dots).length; i++) {
      state.cs[i].y.domain([0, Math.max(max_info*1.03, 0.2)])
      state.cs[i].svg.select('.y.axis').call(c.yAxis)
      state.dots[i].data(infoPlaneData)
      state.dots[i]
      .attr("cx", function(d) {return c.x(d[0])})
      .attr("cy", function(d) {return c.y(d[1])})
    }

    // For the second plot, update the pareto front
    if (state.dots.length > 1) {
      unique_info_vals = tf.concat([info_vals, tf.add(info_vals, 1.)])
      max_vals = []
      max_info_allocs = []
      for (let i=0; i<40; i++) {
        matching_template = tf.where(tf.less(tf.abs(tf.sub(tf.sum(info_vectors, axis=1),tf.gather(unique_info_vals, i))), tf.tensor(0.03)), info_outs_all, tf.zerosLike(info_outs_all))
        max_val = tf.max(matching_template)
        max_ind = tf.argMax(matching_template)
        max_vals.push(max_val)
        max_info_allocs.push(tf.gather(info_vectors, max_ind))

      }

      max_vals = tf.stack(max_vals)
      max_info_allocs = tf.stack(max_info_allocs)

      together = await tf.concat([tf.stack([unique_info_vals, max_vals], 1), max_info_allocs], 1).array()
      var line = d3.line().x(d => state.cs[1].x(d[0])) 
      state.pareto
        .at({d: line.y(d => state.cs[1].y(d[1]))(together)})

      state.pareto0
        .at({d: line.y(d => state.cs[1].y2(d[2]))(together)})
      state.pareto1
        .at({d: line.y(d => state.cs[1].y2(d[3]))(together)})
    }
  }
